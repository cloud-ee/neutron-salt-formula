{%- from "neutron/map.jinja" import node with context %}
{%- set name="neutron" %}
...
keystone_{{ name }}_endpoint:
  keystone.endpoint_present:
    - name: {{ name }}
    - publicurl: http://{{ salt["pillar.get"](name + ":public_ip") }}::9696
    - internalurl: http://{{ salt["pillar.get"](name + ":internal_ip") }}::9696
    - adminurl: http://{{ salt["pillar.get"](name + ":admin_ip") }}::9696
